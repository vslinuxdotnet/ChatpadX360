...
int getbaud = 0;
  ...
  INTCON.GIE = 1; // Global Interrupt Enable/disable
    ...
    INTCON2.RBPU = 0;    //pull-ups enabled on PORTB.
     #else
     OPTION_REG = 0;
     //OPTION_REG.F7 = 0; //pull-ups enabled on PORTB.
     //OPTION_REG.F6 = 0; //Interrupt on falling edge of INT pin
      //RBPU = 0;
    #endif
      ...
      PORTC = 255; //all off
       ...
       Delay_ms(500);
         ...
         //inicial keys pressed functions
          if (justPressed[5]&1){ // 1 pressed at start: change baudrate serial port, one at time increment
  ...
  //UART_Write_text("KEY:")  ;
                 ...
                 {
                      PWM1_Set_Duty(0);//full light on
                      PWM2_Set_Duty(0);//full light on
                      lightStrenght = 0;
                 }
                 IntToHex(bla,XX2);
            ///UART_Write_text(XX2);
